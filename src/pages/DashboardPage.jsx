...
const Dashboard = () => {
  ...
  if (loading) return (
    <div className="flex items-center justify-center h-[80vh]">
      <div className="w-16 h-16 border-4 border-batik-gold border-t-transparent rounded-full animate-spin"></div>
    </div>
  );

  const statCards = [
    { label: 'Revenue Hari Ini', value: formatRupiah(stats?.today_revenue || 0), icon: TrendingUp, color: 'from-batik-gold to-batik-warm', change: '+12%' },
    { label: 'Transaksi Hari Ini', value: stats?.today_transactions || 0, icon: ShoppingCart, color: 'from-batik-green to-batik-dark', change: '+5%' },
    { label: 'Total Produk', value: stats?.total_products || 0, icon: Package, color: 'from-batik-olive to-batik-bronze', change: '' },
    { label: 'Stok Menipis', value: stats?.low_stock_count || 0, icon: AlertTriangle, color: 'from-red-500 to-red-700', change: '', alert: true },
  ];

  return (
    <div className="space-y-8 pb-10">
      {/* Hero Section */}
      <div className="relative overflow-hidden rounded-3xl bg-gradient-to-r from-batik-dark via-batik-green to-batik-dark text-white shadow-xl border border-batik-gold/20">
        <div className="absolute top-0 right-0 p-10 opacity-10 mix-blend-overlay">
           <svg width="200" height="200" viewBox="0 0 200 200" xmlns="http://www.w3.org/2000/svg">
             <path fill="#D4AF37" d="M45.7,-76.3C58.9,-69.3,69.1,-55.6,76.3,-41.2C83.5,-26.8,87.6,-11.7,85.2,2.4C82.8,16.5,73.8,29.7,64.1,41.2C54.4,52.7,44,62.5,31.7,69.3C19.4,76.1,5.2,79.9,-8.3,78.8C-21.8,77.7,-34.6,71.7,-45.5,63.4C-56.4,55.1,-65.4,44.5,-72.1,32.3C-78.8,20.1,-83.2,6.3,-80.4,-6.2C-77.6,-18.7,-67.6,-29.9,-57.3,-39.8C-47,-49.7,-36.4,-58.3,-24.8,-66.3C-13.2,-74.3,-0.6,-81.7,13.6,-80.6C27.9,-79.6,55.8,-70.2,45.7,-76.3Z" transform="translate(100 100)" />
           </svg>
        </div>
        
        <div className="relative z-10 px-8 py-10 flex flex-col md:flex-row items-center justify-between gap-6">
          <div>
             <h1 className="text-3xl md:text-4xl font-serif font-bold text-batik-gold mb-2 drop-shadow-sm">{greeting}, {user?.name}</h1>
             <p className="text-gray-200 max-w-xl text-sm leading-relaxed">
               Selamat datang kembali di Aplikasi Kasir Batik Nusantara. Pantau performa penjualan dan stok produk Anda hari ini dengan mudah.
             </p>
          </div>
          <div className="flex gap-4">
            <div className="bg-white/10 backdrop-blur-md px-6 py-3 rounded-2xl border border-white/20 text-center hover:bg-white/20 transition-colors shadow-lg">
               <p className="text-[10px] text-batik-gold uppercase tracking-[0.2em] font-bold mb-1">Waktu</p>
               <p className="text-2xl font-bold font-mono"><Clock size={16} className="inline mr-1 opacity-70" />{new Date().toLocaleTimeString('id-ID', {hour: '2-digit', minute:'2-digit'})}</p>
            </div>
            <div className="bg-gradient-to-br from-batik-gold to-batik-warm backdrop-blur-md px-6 py-3 rounded-2xl border border-white/20 text-center shadow-[0_0_20px_rgba(212,175,55,0.3)] text-batik-dark">
               <p className="text-[10px] text-batik-dark/80 uppercase tracking-[0.2em] font-bold mb-1">Tanggal</p>
               <p className="text-2xl font-bold font-serif">{new Date().getDate()}</p>
            </div>
          </div>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
        {statCards.map((card, i) => (
          <div key={i} className="bg-white rounded-2xl p-6 shadow-sm border border-gray-100 hover:border-batik-gold/40 hover:shadow-[0_8px_30px_rgb(0,0,0,0.08)] hover:-translate-y-1 transition-all duration-300 group">
            <div className="flex items-start justify-between mb-4">
              <div className={`w-14 h-14 bg-gradient-to-br ${card.color} rounded-2xl flex items-center justify-center shadow-lg group-hover:rotate-6 transition-transform duration-300`}>
                <card.icon size={24} className="text-white" />
              </div>
              {card.change && (
                <span className="text-xs font-bold text-batik-green bg-batik-green/10 px-2.5 py-1 rounded-full flex items-center gap-1 border border-batik-green/20">
                  <ArrowUpRight size={12} strokeWidth={3} />{card.change}
                </span>
              )}
            </div>
            <p className="text-3xl font-bold text-gray-800 tracking-tight">{card.value}</p>
            <p className="text-sm text-gray-500 font-medium mt-1 uppercase tracking-wide text-[11px]">{card.label}</p>
          </div>
        ))}
      </div>
      ...
      <div className="grid lg:grid-cols-3 gap-8">
        ...
        <div className="lg:col-span-2 bg-white rounded-2xl p-8 shadow-sm border border-gray-100">
          <div className="flex items-center justify-between mb-6">
             ...
             <select className="bg-gray-50 border border-gray-200 text-sm rounded-lg px-3 py-1.5 outline-none focus:ring-2 focus:ring-batik-gold/50">
        ...
        <div className="bg-white rounded-2xl p-8 shadow-sm border border-gray-100 flex flex-col">
          <div className="flex items-center justify-between mb-6">
            ...
            <Link to="/reports" className="text-xs font-bold text-batik-gold hover:underline uppercase tracking-wide">Lihat semua</Link>
          </div>
            ...
            {(stats?.top_products || []).slice(0, 5).map((p, i) => (
              <div key={i} className="flex items-center gap-4 group cursor-pointer hover:bg-gray-50 p-2 rounded-xl transition-colors">
                ...
                <div className="flex-1 min-w-0">
                  <p className="text-sm font-semibold text-gray-800 truncate group-hover:text-batik-gold transition-colors">{p.product_name}</p>
                  <p className="text-xs text-gray-500">{p.total_qty} terjual minggu ini</p>
                </div>
                <div className="text-right">
                   <p className="text-sm font-bold text-batik-dark">{formatRupiah(p.total_revenue)}</p>
                </div>
              </div>
            ))}
          </div>
      ...
      <div className="grid lg:grid-cols-2 gap-8">
        ...
        <div className="bg-white rounded-2xl p-8 shadow-sm border border-gray-100">
          <div className="flex items-center justify-between mb-6">
            ...
            <Link to="/transactions" className="text-xs font-bold text-batik-gold hover:underline uppercase tracking-wide">Lihat semua</Link>
          </div>
          <div className="space-y-4">
            {(stats?.recent_transactions || []).slice(0, 5).map(trx => (
              <div key={trx.id} className="flex items-center gap-4 p-4 rounded-2xl hover:bg-[#F8F5F2] transition-colors border border-transparent hover:border-gray-200">
                ...
                <div className="text-right">
                  <p className="text-base font-bold text-batik-dark">{formatRupiah(trx.total)}</p>
            ...
            {(stats?.low_stock_products || []).slice(0, 5).map(p => (
              <div key={p.id} className="flex items-center gap-4 p-4 rounded-2xl bg-white border border-red-100 shadow-sm hover:shadow-md transition-shadow">
                ...
                <div className="text-right">
                  ...
                  <p className="text-lg font-bold text-batik-olive">{p.min_stock}</p>
                </div>
              </div>
            ))}
